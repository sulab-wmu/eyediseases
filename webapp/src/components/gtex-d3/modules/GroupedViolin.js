...
*/
import $ from "jquery"
import {ascending, extent, max, median, min, quantile} from "d3-array";
...
import {kernel, kernelBandwidth, kernelDensityEstimator} from "./kde";
        ...
        this.groups.forEach((g) => {
            ...
            if (showSize) {
                const sizeScale = scaleBand()
                    .domain(entries.map((d) => {
                        return d.size == undefined ? '(0)' : `(${d.size || 0})`
                    }))
                    .rangeRound([this.scale.x(group), this.scale.x(group) + this.scale.x.bandwidth()]);
