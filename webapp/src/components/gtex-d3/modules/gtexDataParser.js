...
export function parseDynEqtl(json) {
    ...
    ['data', 'genotypes', 'pValue', 'pValueThreshold', 'tissueSiteDetailId'].forEach((d) => {
        if (!Object.prototype.hasOwnProperty.call(json, d)) {
            console.error(json);
...
export function parseSingleTissueEqtls(data, tissueSiteTable = undefined) {
    ...
    if (!Object.prototype.hasOwnProperty.call(data, attr)) throw "Parsing Error: required attribute is not found: " + attr;
    ['variantId', 'tissueSiteDetailId', 'nes', 'pValue'].forEach((k) => {
        if (!Object.prototype.hasOwnProperty.call(data[attr][0], k)) throw 'Parsing Error: required attribute is missing: ' + attr;
    });
...
export function parseGenes(data, single = false, geneId = null) {
    const attr = 'gene';
    if (!Object.prototype.hasOwnProperty.call(data, attr)) throw "Parsing Error: attribute gene doesn't exist.";
...
export function parseTissues(json) {
    ...
    if (!Object.prototype.hasOwnProperty.call(json, attr)) throw 'Parsing Error: required json attr is missing: ' + attr;
    const tissues = json[attr];

    // sanity check
    ['tissueSiteDetailId', 'tissueSiteDetail', 'colorHex'].forEach((d) => {
        if (!Object.prototype.hasOwnProperty.call(tissues[0], d)) throw 'Parsing Error: required json attr is missing: ' + d;
    });
...
export function parseTissueSampleCounts(json) {
    ...
    if (!Object.prototype.hasOwnProperty.call(tissue, 'tissueSiteDetailId')) throw 'Parsing Error: required attr is missing: tissueSiteDetailId';
    if (!Object.prototype.hasOwnProperty.call(tissue, 'rnaSeqAndGenotypeSampleCount')) throw 'Parsing Error: required attr is missing: rnaSeqAndGenotypeSampleCount';
    return tissues;
}
...
export function parseTissueSites(data, forEqtl = false) {
    ...
    const attr = 'tissueInfo';
    if (!Object.prototype.hasOwnProperty.call(data, attr)) throw 'Parsing Error: required json attribute is missing: ' + attr;
    let tissues = data[attr];
    ['tissueSite', 'tissueSiteDetailId', 'tissueSiteDetail'].forEach((d) => {
        if (!Object.prototype.hasOwnProperty.call(tissues[0], d)) throw `parseTissueSites attr error. ${d} is not found`;
    });
    ...
    let tissueGroups = tissues.reduce((arr, d) => {
        ...
        if (!Object.prototype.hasOwnProperty.call(arr, groupName)) arr[groupName] = []; // initiate an array
...
export function parseModelExons(json, isFull) {
    ...
    // sanity check
    ['start', 'end'].forEach((d) => {
        if (!Object.prototype.hasOwnProperty.call(json[attr][0], d)) throw 'Parsing Error: Required json attribute is missing: ' + d;
    });
...
export function parseJunctions(json) {
    ...
    if (!Object.prototype.hasOwnProperty.call(json, attr)) throw 'Parsing Error: parseJunctions input error. ' + attr;

    // check required json attributes
    ['tissueSiteDetailId', 'junctionId'].forEach((d) => {
        // use the first element in the json objects as a test case
        if (json[attr].length && !Object.prototype.hasOwnProperty.call(json[attr][0], d)) {
...
export function parseExons(json) {
    const attr = 'exon';
    if (!Object.prototype.hasOwnProperty.call(json, attr)) throw 'Parsing Error: required json attribute is missing: exon';
...
export function parseTranscripts(json) {
    ...
    if (!Object.prototype.hasOwnProperty.call(json, attr)) throw('parseIsoforms input error');

    // check required attributes, use the first transcript as the test case
    ['transcriptId', 'start', 'end'].forEach((k) => {
        if (!Object.prototype.hasOwnProperty.call(json[attr][0], k)) {
            // console.error(d);
            throw 'Parsing Error: required json attribute is missing: ' + k
        }
    });
...
export function parseExonExpression(data, exons) {
    ...
    if (!Object.prototype.hasOwnProperty.call(data, attr)) throw('parseExonExpression input error');

    const exonObjects = data[attr];
    // error-checking
    ['median', 'exonId', 'tissueSiteDetailId'].forEach((d) => {
        if (!Object.prototype.hasOwnProperty.call(exonObjects[0], d)) throw 'Fatal Error: parseExonExpression attr not found: ' + d;
    });
    ...
    exonObjects.forEach((d) => {
        ...
        // error-checking
        ['end', 'start'].forEach((p) => {
            if (!Object.prototype.hasOwnProperty.call(exon, p)) throw 'Fatal Error: parseExonExpression position attr not found: ' + p;
        });
...
export function parseJunctionExpression(data) {
    const attr = 'medianJunctionExpression';
    if (!Object.prototype.hasOwnProperty.call(data, attr)) throw('parseJunctionExpression input error');
...
export function parseTranscriptExpression(data) {
    ...
    if (!Object.prototype.hasOwnProperty.call(data, attr)) throw('Parsing Error: parseTranscriptExpression input error');
...
export function parseMedianExpression(data) {
    ...
    ['median', 'tissueSiteDetailId', 'gencodeId'].forEach((d) => {
        if (!Object.prototype.hasOwnProperty.call(data[attr][0], d)) {
...
export function parseGeneExpressionForBoxplot(data, tissues = undefined, colors = undefined) {
    ...
    if (!Object.prototype.hasOwnProperty.call(data, attr)) throw(`Parsing error: required JSON attribute ${attr} missing.`);
