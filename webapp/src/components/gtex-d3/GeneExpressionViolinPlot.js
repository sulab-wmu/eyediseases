...
import {ascending, median} from 'd3-array';
...
export function launch(rootId, tooltipRootId, gencodeId, plotTitle = "Gene Expression Violin Plot", urls = getGtexUrls(), margins = _setViolinPlotMargins(50, 75, 250, 60), dimensions = {
    w: window.innerWidth * 0.8,
    h: 250
}) {
    console.log(urls.tissue)
    console.log(urls.geneExp)
    const promises = [
...
function _addToolbar(vplot, tooltip, ids, urls) {
    ...
    let tissueFilterButton = select(`#${ids.buttons.filter}`)
        .on('mouseover', () => {
            toolbar.tooltip.show('Filter Tissues');
        })
        .on('mouseout', () => {
...
function _parseGeneExpressionForViolin(data, idNameMap = undefined, colors = undefined, useLog = true) {
    ...
    if (!Object.prototype.hasOwnProperty.call(data, attr)) throw 'Parse Error: required json attribute is missing: ' + attr;
    data[attr].forEach((d) => {
